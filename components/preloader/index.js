  ...
  return (
    <div className="container mx-auto mb-14">
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 2xl:grid-cols-5 text-center gap-8">
        {length &&
          arraFromNumber(length).map((item) => (
            <div
              className="w-[250px] sm:w-full mx-auto p-8 border shadow-lg rounded-2xl"
              key={item}
            >
...
export const PublicationListPreloader = ({ length }) => {
  return (
    <div className="container mx-auto mb-14">
          ...
          arraFromNumber(length).map((item) => (
            <div
              ...
              className={`px-2 py-4 cursor-pointer ${
                item + 1 < length ? "border-b" : ""
              }`}
            >
              <div className="rounded-full bg-slate-200 h-2 w-1/3 mb-2" />
              <div className="rounded-full bg-slate-200 h-2 w-[150px] mb-2" />
              <div className="rounded-full bg-slate-200 h-2 w-3/4 mb-2" />
              <div className="rounded-full bg-slate-200 h-2 w-1/2" />
            </div>
          ))}
      </div>
    </div>
